...
gap:"8px","font-family":"Calibri,Helvetica,sans-serif","max-width":"100%"}),768>window.innerWidth&&(a.chatBotBtn.css({right:"25px"}),a.chatBotBtnTips.css({padding:"10px",width:"60%",height:"initial"})),a.chatBotBtnTips.find(".beckon-icon").css({"box-sizing":"border-box",width:"18px",height:"21px",verticalAlign:"middle","flex-shrink":0}),a.chatBotBtnTips.find(".close-icon").css({"box-sizing":"border-box",width:"10px",height:"10px",cursor:"pointer","flex-shrink":0}),d.yield(a.setBtnSize(),2);(c=a.getPopupTipsVisible())&&
...
return new Promise(function(c){var d=a.chatBotBtn.find("img")[0];d.complete&&(a.chatBotBtn.css({width:d.naturalWidth*b/d.naturalHeight+"px"}),console.log("iconImg complete"),c());d.addEventListener("load",function(){a.chatBotBtn.css({width:d.naturalWidth*b/d.naturalHeight+"px"});console.log("iconImg load");c()});d.onerror=function(){console.log("iconImg onerror");c()}})},showChatBotBtnTips:function(){var a=this,b=this.chatBotBtn[0].getBoundingClientRect();this.parent.append(this.chatBotBtnTips);this.chatBotBtnTips.fadeIn();
this.isChatBotBtnTipsVisible=!0;this.chatBotBtnTips.css({display:"flex","box-sizing":"border-box",position:"absolute",bottom:window.innerHeight-b.bottom+"px",right:window.innerWidth-b.left+8+"px",cursor:"pointer"});this.chatBotBtnTips[0].addEventListener("click",function(c){c.target===a.chatBotBtnTips.find(".close-icon")[0]?(a.hidePopupTips(),BookEvent.trigger("clickButton",[BookInfo.getCurrentPageIndex(),"chatbot","close chatBot tips"])):a.beforeShow()})},handleBtnDrag:function(a){function b(n){this.isDraging=
...
a.on("tap",function(n){g&&g(n)})},hidePopupTips:function(){this.chatBotBtnTips&&(this.isChatBotBtnTipsVisible=!1,this.chatBotBtnTips.fadeOut(),this.setPopupTipsVisible("false"))},getPopupTipsVisible:function(){var a=getHashParams("chatbotVisible");if(a&&"true"==a)return!1;try{var b=window.localStorage.getItem("isChatbotPopupTipsVisible");return b?JSON.parse(b):!0}catch(c){}},setPopupTipsVisible:function(a){try{window.localStorage.setItem("isChatbotPopupTipsVisible",a)}catch(b){}},handleBoxDrag:function(a){function b(m){this.isDraging=
!0;var l=g.getBoundingClientRect();m=m.center;h=m.x-l.x;k=m.y-l.y;$(e).css({cursor:"move",width:"100%",height:"100%"})}function c(m){if(this.isDraging){var l=m.center;m=f.width();var n=f.height(),p=$(g).outerWidth(),q=$(g).outerHeight(),r=l.x-h;l=l.y-k;r=Math.min(m-p,Math.max(0,r));l=Math.min(n-q,Math.max(0,l));$(g).css({bottom:"initial",right:"initial",left:r+"px",top:l+"px"})}}function d(){this.isDraging=!1;$(e).css({cursor:"pointer",width:"80%",height:"50px"})}var e=a.$ele,f=a.parent,g=a.$box;
...
return bookConfig.RightToLeft||c},initHtml:function(){this.container=$('<div class="chatbot-container"></div>');this.chatbotBox=$('<div id="chatbotBox" class="chatbot-box"></div>');this.chatbotDrag=$('<div id="draggableBox" class="chatbot-drag"></div>');this.chatbotResize=$('<div id="resizableBox" class="chatbot-resize"></div>');this.resizeLeft=$('<div class="chatbot-resize-item chatbot-resize-left"></div>');this.resizeRight=$('<div class="chatbot-resize-item chatbot-resize-right"></div>');this.resizeTop=
$('<div class="chatbot-resize-item chatbot-resize-top"></div>');this.resizeBottom=$('<div class="chatbot-resize-item chatbot-resize-bottom"></div>');this.resizeMask=$('<div class="chatbot-resize-mask"></div>');var a=_get(window,"website_domian_wc","https://fliphtml5.com"),b=removeUrlParts(window.location.href);bookConfig.socialShareLink&&(b=bookConfig.socialShareLink);var c=getMetaTagValueByName("description"),d=getMetaTagValueByName("keywords");a+="/chatbot/chatbot.php";b=Metacharacter(b);d=Metacharacter(d);
c=Metacharacter(c);var e=this.isRtl()?"1":"0",f,g,h=(null==(f=selectedLanguage)?void 0:null==(g=f.language)?void 0:g.toLowerCase())||"english";this.frame=$("<iframe id='chatbotIframe' allow=\"microphone\" class='chatbot-frame' src='"+(a+"?url="+b+"&desc="+c+"&keyword="+d+"&isRtl="+e+"&lang="+h)+"' ></iframe>");this.container.append(this.chatbotBox);this.container.append(this.chatBotBtn);this.parent.append(this.container);this.chatbotBox.css({"box-sizing":"border-box","z-index":9999,position:"absolute",
right:"120px",bottom:"50px",width:"421px",height:"740px",opacity:0,visibility:"hidden","border-radius":"12px"});this.chatbotResize.append(this.frame);this.chatbotResize.append(this.resizeLeft);this.chatbotResize.append(this.resizeRight);this.chatbotResize.append(this.resizeTop);this.chatbotResize.append(this.resizeBottom);this.chatbotResize.append(this.resizeMask);this.chatbotBox.append(this.chatbotDrag);this.chatbotBox.append(this.chatbotResize);this.initResizeCss();this.chatbotDrag.css({width:"80%",
left:"50%",transform:"translateX(-50%)"})},initEvent:function(){var a=this;window.addEventListener("message",function(b){b=(null==b?void 0:b.data)||{};var c=void 0===b.eventName?"":b.eventName,d=void 0===b.data?{}:b.data;if("chatbot"===(void 0===b.type?"":b.type).toLowerCase())switch(c){case "close":a.hide();break;case "changePage":d.page&&gotoPageFun(d.page)}});this.handleResize()},handleResize:function(){function a(l,n){function p(t){this.isResizing=!0;g=t.center.x;h=t.center.y;k=c.width();m=c.height();
switch(n){case "right":c.css({right:"initial",left:t.center.x-k+"px"});break;case "left":c.css({right:"initial",left:t.center.x+"px"});break;case "top":c.css({bottom:"initial",top:t.center.y+"px"});break;case "bottom":c.css({bottom:"initial",top:t.center.y-m+"px"})}$(l).css({cursor:"move"});d.css({display:"block"});b.frame.css({"box-shadow":"none",border:"none"})}function q(t){if(this.isResizing){var x=Math.min(t.center.x-g,e),v=Math.min(t.center.y-h,f);if(!(t.center.x>e||t.center.y>f||0>t.center.x||
0>t.center.y))switch(n){case "right":c.css({width:k+x+"px"});break;case "left":c.css({left:t.center.x+"px",width:k-x+"px"});break;case "top":c.css({top:t.center.y+"px",height:m-v+"px"});break;case "bottom":c.css({height:m+v+"px"})}}}function r(){this.isResizing=!1;"left"!=n&&"right"!=n||$(l).css({cursor:"ew-resize"});"top"!=n&&"bottom"!=n||$(l).css({cursor:"ns-resize"});d.css({display:"none"});b.frame.css({"box-shadow":"0 0 20px 0 rgba(0, 0, 0, 0.1)",border:"1px solid rgba(0,0,0,0.1)"})}var u=new Hammer(l);
u.add(new Hammer.Pan({threshold:0,pointers:0}));u.on("panstart panmove panend",function(t){t.preventDefault();switch(t.type){case "panstart":p(t);break;case "panmove":q(t);break;case "panend":r(t)}})}document.querySelectorAll(".chatbot-resize-item").forEach(function(l){var n=l.classList[1].split("-")[2];a(l,n)});var b=this,c=this.chatbotBox,d=this.resizeMask,e=this.parent.width(),f=this.parent.height(),g=0,h=0,k=0,m=0},initResizeCss:function(){this.resizeMask.css({display:"none",position:"absolute",
left:0,top:0,right:0,bottom:0,"z-index":1E3});this.resizeLeft.css({position:"absolute",left:0,top:0,bottom:0,width:"5px",height:"100%",cursor:"ew-resize","z-index":1E3});this.resizeRight.css({position:"absolute",right:0,top:0,bottom:0,width:"5px",height:"100%",cursor:"ew-resize","z-index":1E3});this.resizeTop.css({position:"absolute",left:0,top:0,right:0,height:"5px",width:"100%",cursor:"ns-resize","z-index":1E3});this.resizeBottom.css({position:"absolute",left:0,bottom:0,right:0,width:"100%",height:"5px",
cursor:"ns-resize","z-index":1E3})},showOrHide:function(){this.visible?this.hide():this.show()},show:function(){this.visible||(this.visible=!0,this.chatbotBox.css({opacity:1,visibility:"visible"}),this.handleBoxDrag({$ele:this.chatbotDrag[0],parent:this.parent,$box:this.chatbotBox[0]}),this.postMessage({messageType:"show"}))},hide:function(){this.visible&&(this.visible=!1,this.chatbotBox.css({opacity:0,visibility:"hidden"}),this.postMessage({messageType:"hide"}))},postMessage:function(a){var b=document.querySelector("#chatbotIframe");
b&&b.contentWindow.postMessage(a,"*")},onResize:function(a,b){a=this.chatbotBox.height();this.chatbotBox.css({height:Math.min(b-80,a)+"px"})},changeLanguage:function(a){this.chatBotBtnTips&&this.chatBotBtnTips.find("span").text(getLanguage("chatbotBtnTips","Hi~ Ask about the book, or anything!"));this.postMessage({eventName:"changeLang",type:"chatBot",data:{lang:a}})}});
...
this.edtPageNumber=$("<div class='pageNumber' id='tfPageIndex'><label><input aria-labelledby='currentPageIndex' type='text' id='currentPageIndexTextField' autocomplete='off' size='5' value='' /></label></div>");this.parent.append(this.logoBar);this.parent.append(this.topRightBar);bookConfig.appLogoIcon||this.logoBar.css({display:"none",height:"0"});!bookConfig.aboutButtonVisible&&window.userInfo&&0<window.userInfo.c&&this.topRightBar.css({display:"none",height:"0"});this.bottomBar.append(this.buttonBar);
this.bottomBar.append(this.bottomLeftBar).append(this.bottomRightBar);this.bottomLeftBar.append(this.edtPageNumber);this.parent.append(this.bottomBar);this.initPageNumber()},initPageNumber:function(){var a=this.edtPageNumber.find("input"),b=parseInt(bookConfig.totalPageCount),c=16;""==bookConfig.pageNumberCaption&&(1E4<=b&&(c=16),1E4>b&&(c=10),1E3>b&&(c=7),100>b&&(c=4),10>b&&(c=3));a.attr("size",c);Color(this.color.add(120));a.css({color:bookConfig.iconColor,background:"transparent",border:"none"});
this.edtPageWidth=this.edtPageNumber.width()},initExtendedbar:function(){this.extendedBar=new PCExtendedBar(this.parent);global.moreBar=this.extendedBar},resetToolbarColor:function(){this.color=Color(bookConfig.toolbarColor);this.topBarAlpha=Math.min(bookConfig.topToolBarAlpha,bookConfig.ToolBarAlpha);this.extendedBar&&this.extendedBar.$bar.css({"background-color":this.color.rgba(this.topBarAlpha)});this.resetButtonTitleColor();global.PcTelephoneNumberFrame&&global.PcTelephoneNumberFrame.setColor({color:bookConfig.toolbarColor,
textColor:bookConfig.iconFontColor})},setButtonTitle:function(a){if("WeChatShareButton"==a.getClassName())global.WeChatShareImg=new WeChatShare({parent:this.parent,color:"#fff",textColor:bookConfig.toolbarColor,margin:10,padding:10,arrowHeight:12,arrowWidth:12});else if("TelephoneNumberButton"==a.getClassName())global.PcTelephoneNumberFrame=new PcTelephoneNumberBubble({parent:this.parent,color:this.color,textColor:a.fontColor,margin:4,padding:15,arrowHeight:8,arrowWidth:8});else{var b=new TextBubble({parent:this.parent,
text:a.caption,direction:TextBubble.TOP,color:this.color,textColor:a.fontColor,margin:7});a.setTitle(b)}},addTopRightButton:function(){this.topRightButton||(this.topRightButton=$("<div class='topRightButton' id='createButton'></div>"),bookConfig.ToolBarVisible?(this.topRightBar.append(this.topRightButton),this.onResize()):this.parent.append(this.topRightButton))},removeTopRightButton:function(){this.topRightButton&&(this.topRightButton.remove(),this.topRightButton=null)},onResize:function(a,b){if(bookConfig.ToolBarVisible){a=
void 0==a?this.parent.width():parseInt(a);void 0==b?this.parent.height():parseInt(b);b=a/4;var c=a-b-a/4,d=this.edtPageWidth+150+20;c<d&&(c=d,b=(a-c)/2);this.logoBar.width()+20>b?this.logoBar.css({display:"none"}):this.logoBar.css({display:"block"});this.buttonBarWidth=c;this.resetButtonPosition()}},afterButtonClick:function(a){this.extendedBar.visible&&"btnSocialShare"!=a.language.key&&(a!=this.btnMore?isPhone()||isPad()||this.extendedBar.hide():(a=this.buttonsInExtendedBar(),a.each(function(b){b.changeColor()}),
a=null))},resetButtonPosition:function(){(new PCButtonQueue({buttons:this.buttons,buttonWidth:30,buttonTop:5,color:this.color,bottomBar:{leftBar:this.bottomLeftBar,rightBar:this.bottomRightBar},container:{body:this.buttonBar,topRightBar:this.topRightBar,width:this.buttonBarWidth},extended:this.extendedBar.$bar,edtNumber:{body:this.edtPageNumber,width:this.edtPageWidth,top:8,space:5}})).sort();this.extendedBar.resize(this.buttonsInExtendedBar());this.extendedBar.hide();this.buttons.each(function(a){a.changeColor()}.bind(this))}}).extend("Toolbar");
